  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 1000, background: 'rgba(0,0,0,0.4)', backdropFilter: 'blur(4px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}
      ...
      <motion.div initial={{ opacity: 0, scale: 0.95, y: 12 }} animate={{ opacity: 1, scale: 1, y: 0 }} transition={{ duration: 0.18 }}
        style={{ width: '100%', maxWidth: 480, background: 'var(--bg-elevated)', border: '1px solid var(--border)', borderRadius: 20, boxShadow: 'var(--shadow-xl)', overflow: 'hidden' }}>
        ...
        <form onSubmit={submit} style={{ padding: '20px 24px', display: 'flex', flexDirection: 'column', gap: 16 }}>
          <div>
            ...
            <input className="input" value={dossier.titre} readOnly style={{ fontSize: 13, background: 'rgba(255,255,255,0.02)', color: 'var(--text-tertiary)', cursor: 'not-allowed' }} />
          </div>
          <div>
                ...
                return (
                  <motion.div key={d.id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.04 }}
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '16px 20px', cursor: 'pointer', transition: 'background 0.12s' }}
                      ...
                      <div style={{ width: 42, height: 42, borderRadius: 12, background: 'rgba(99,102,241,0.08)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, fontSize: 20 }}>📁</div>
                      ...
                      <div style={{ display: 'flex', gap: 6, flexShrink: 0 }}>
                        {perm.ajouter  && <span style={{ fontSize: 11, padding: '2px 9px', borderRadius: 99, background: 'rgba(16,185,129,0.10)', color: '#34D399', fontWeight: 600 }}>✓ Ajouter</span>}
                        {perm.supprimer && <span style={{ fontSize: 11, padding: '2px 9px', borderRadius: 99, background: 'rgba(239,68,68,0.10)', color: '#F87171', fontWeight: 600 }}>✓ Supprimer</span>}
                        {!perm.ajouter && !perm.supprimer && <span style={{ fontSize: 11, padding: '2px 9px', borderRadius: 99, background: 'rgba(255,255,255,0.06)', color: 'var(--text-secondary)', fontWeight: 600 }}>👁 Lecture seule</span>}
                    ...
                    <AnimatePresence>
                      {isExpanded && (
                        <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.2 }}
                          ...
                          {/* Action bar */}
                          <div style={{ display: 'flex', gap: 10, padding: '12px 20px', background: 'rgba(255,255,255,0.02)', alignItems: 'center' }}>
                            {perm.ajouter && (
                              <label style={{ display: 'inline-flex', alignItems: 'center', gap: 7, padding: '7px 14px', borderRadius: 9, background: 'var(--accent)', color: '#fff', fontSize: 13, fontWeight: 600, cursor: 'pointer', transition: 'opacity 0.15s' }}
                            ...
                            <button onClick={e => { e.stopPropagation(); setSoumModal(d) }}
                              style={{ display: 'inline-flex', alignItems: 'center', gap: 7, padding: '7px 14px', borderRadius: 9, background: 'rgba(99,102,241,0.08)', color: 'var(--accent-bright)', border: '1px solid rgba(99,102,241,0.25)', fontSize: 13, fontWeight: 600, cursor: 'pointer', transition: 'all 0.15s' }}
                          ...
                          {isLoading ? (
                            ...
                            <>
                              <div style={{ display: 'grid', gridTemplateColumns: '1fr 80px 120px 80px 80px', gap: 10, padding: '8px 20px', borderTop: '1px solid var(--border)', background: 'rgba(255,255,255,0.02)' }}>
                              ...
                              {dFiles.map(f => (
                                <div key={f.id} style={{ display: 'grid', gridTemplateColumns: '1fr 80px 120px 80px 80px', gap: 10, alignItems: 'center', padding: '10px 20px', borderTop: '1px solid var(--border)', transition: 'background 0.12s' }}
                                  ...
                                  <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                                    ...
                                    {f.status === 'en_attente' && (
                                      <span style={{ flexShrink: 0, fontSize: 10, fontWeight: 600, padding: '2px 7px', borderRadius: 99, background: 'rgba(245,158,11,0.10)', color: '#FBBF24', border: '1px solid rgba(245,158,11,0.20)' }}>
                                        En attente
                                      </span>
                                    )}
                                  </div>
                                  ...
                                  <div style={{ display: 'flex', gap: 5 }}>
                                    <a href={f.fichier} target="_blank" rel="noreferrer" title="Voir / Télécharger"
                                      style={{ width: 28, height: 28, borderRadius: 7, border: '1px solid var(--border)', background: 'var(--bg-base)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--text-secondary)', textDecoration: 'none', fontSize: 13 }}>
                                      👁
                                    </a>
                                    {perm.supprimer && (
