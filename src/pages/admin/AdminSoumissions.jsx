  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 1200, background: 'rgba(0,0,0,0.45)', backdropFilter: 'blur(4px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}
      ...
      <motion.div initial={{ opacity: 0, scale: 0.96 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.16 }}
        style={{ width: '100%', maxWidth: 420, background: 'var(--bg-elevated)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 18, boxShadow: '0 24px 80px rgba(0,0,0,0.7)', overflow: 'hidden' }}>
    ...
    <>
      ...
      <motion.div
        ...
        style={{
          ...
          background: 'var(--bg-elevated)', borderLeft: '1px solid rgba(255,255,255,0.06)',
      ...
      >
        ...
        <div style={{ padding: '16px 20px', borderBottom: '1px solid rgba(255,255,255,0.06)', display: 'flex', alignItems: 'center', gap: 10, flexShrink: 0 }}>
          ...
          <button onClick={onClose}
            style={{ width: 30, height: 30, borderRadius: 8, border: '1px solid rgba(255,255,255,0.06)', background: 'rgba(255,255,255,0.02)', cursor: 'pointer', color: 'var(--text-tertiary)', fontSize: 18, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
            ×
          </button>
        </div>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '20px', display: 'flex', flexDirection: 'column', gap: 20 }}>
          ...
          <section>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '14px', background: 'rgba(255,255,255,0.02)', borderRadius: 12, border: '1px solid rgba(255,255,255,0.06)' }}>
              <div style={{ width: 40, height: 40, borderRadius: '50%', 'background': 'rgba(99,102,241,0.10)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--accent)', fontSize: 14, fontWeight: 700, flexShrink: 0 }}>
                {initials}
              </div>
              <div>
          ...
          <section>
            ...
            <div style={{ padding: '14px', background: 'rgba(255,255,255,0.02)', borderRadius: 12, border: '1px solid rgba(255,255,255,0.06)', display: 'flex', flexDirection: 'column', gap: 10 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                <span style={{ fontSize: 18 }}>📁</span>
                <span style={{ fontSize: 14, fontWeight: 600, color: 'var(--text-primary)' }}>{dossierNom}</span>
              </div>
              {sub.commentaire ? (
                <div style={{ fontSize: 13, color: 'var(--text-secondary)', lineHeight: 1.55, padding: '10px 12px', background: 'rgba(255,255,255,0.02)', borderRadius: 8, borderLeft: '3px solid var(--border-mid)' }}>
                  {sub.commentaire}
                </div>
              ) : (
          ...
          <section>
            ...
            {loadingFiles ? (
              ...
              <div style={{ padding: '24px', textAlign: 'center', color: 'var(--text-tertiary)', fontSize: 13, background: 'rgba(255,255,255,0.02)', borderRadius: 12, border: '1px solid rgba(255,255,255,0.06)' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {fichiers.map(f => {
                  ...
                  const btnBase = { flexShrink: 0, borderRadius: 7, border: '1px solid rgba(255,255,255,0.06)', background: 'var(--bg-elevated)', color: 'var(--text-secondary)', fontSize: 12, fontWeight: 500, textDecoration: 'none', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'all 0.15s', cursor: 'pointer', padding: '5px 9px', gap: 4 }
                  return (
                    <div key={f.id} style={{ display: 'flex', alignItems: 'center', gap: 11, padding: '11px 14px', background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 10, transition: 'border-color 0.15s' }}
        ...
        {sub.status === 'en_attente' ? (
          <div style={{ padding: '16px 20px', borderTop: '1px solid rgba(255,255,255,0.06)', display: 'flex', gap: 10, flexShrink: 0, background: 'var(--bg-elevated)' }}>
            <button onClick={() => onRefuse(sub)}
              style={{ flex: 1, padding: '10px 0', borderRadius: 9, border: '1px solid rgba(220,38,38,0.35)', background: 'rgba(220,38,38,0.06)', color: '#F87171', fontSize: 14, fontWeight: 600, cursor: 'pointer', transition: 'all 0.15s' }}
              onMouseEnter={e => e.currentTarget.style.background = 'rgba(220,38,38,0.14)'}
              onMouseLeave={e => e.currentTarget.style.background = 'rgba(220,38,38,0.06)'}>
              ❌ Refuser
            </button>
            <button onClick={handleApprove} disabled={busy}
              style={{ flex: 2, padding: '10px 0', borderRadius: 9, border: 'none', background: 'var(--green)', color: 'white', fontSize: 14, fontWeight: 600, cursor: busy ? 'not-allowed' : 'pointer', opacity: busy ? 0.65 : 1, transition: 'opacity 0.15s' }}>
              {busy ? 'Traitement…' : '✅ Accepter la soumission'}
            </button>
          </div>
        ) : (
          <div style={{ padding: '14px 20px', borderTop: '1px solid rgba(255,255,255,0.06)', display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0, background: 'var(--bg-elevated)' }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0 }}>
        ...
        <main style={{ flex: 1, padding: '28px 32px 48px', overflowY: 'auto' }}>
          ...
          <div style={{ marginBottom: 24 }}>
            <h1 style={{ fontFamily: 'var(--font-display)', fontSize: 22, fontWeight: 700, letterSpacing: '-0.04em', color: 'var(--text-primary)', margin: '0 0 4px' }}>Soumissions</h1>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 14, marginBottom: 24 }}>
            {[
              { label: 'Total',      value: counts.all,        color: 'var(--accent-bright)', bg: 'rgba(99,102,241,0.10)', icon: '📋' },
              { label: 'En attente', value: counts.en_attente, color: '#FBBF24', bg: 'rgba(245,158,11,0.10)',  icon: '⏳' },
              { label: 'Acceptées',  value: counts.approuve,   color: '#34D399', bg: 'rgba(16,185,129,0.10)',  icon: '✅' },
              { label: 'Refusées',   value: counts.rejete,     color: '#F87171', bg: 'rgba(239,68,68,0.10)',  icon: '❌' },
            ].map(c => (
          ...
          <div style={{ display: 'flex', gap: 6, marginBottom: 18, flexWrap: 'wrap' }}>
            {[
              ...
              <button key={tab.key} onClick={() => setFilter(tab.key)} style={{
                ...
                background:   filter === tab.key ? 'rgba(99,102,241,0.15)' : 'var(--glass-bg)',
                color:        filter === tab.key ? 'var(--accent-bright)' : 'var(--text-secondary)',
                borderColor:  filter === tab.key ? 'rgba(99,102,241,0.30)' : 'var(--border-mid)',
              }}>{tab.label}</button>
            ))}
          </div>
          ...
          {loading ? (
            ...
            <div className="surface" style={{ overflow: 'hidden' }}>
              {/* Header */}
              <div style={{ display: 'grid', gridTemplateColumns: '1.4fr 1.3fr 1.6fr 90px 100px 130px', gap: 12, padding: '10px 18px', borderBottom: '1px solid rgba(255,255,255,0.06)', background: 'rgba(255,255,255,0.02)' }}>
              ...
              {filtered.map((s, i) => (
                ...
                >
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 9 }}>
                    <div style={{ width: 30, height: 30, borderRadius: '50%', 'background': 'rgba(99,102,241,0.10)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--accent)', fontSize: 11, fontWeight: 700, flexShrink: 0 }}>
                    ...
                    {s.status === 'en_attente' ? (
                      <>
                        <button onClick={() => approve(s.id)}
                          style={{ padding: '5px 10px', borderRadius: 7, border: '1px solid rgba(16,185,129,0.30)', background: 'rgba(16,185,129,0.08)', color: '#34D399', fontSize: 12, fontWeight: 600, cursor: 'pointer', transition: 'all 0.15s', whiteSpace: 'nowrap' }}
                        ...
                        <button onClick={() => setRefuseTarget(s)}
                          style={{ padding: '5px 10px', borderRadius: 7, border: '1px solid rgba(220,38,38,0.3)', background: 'rgba(220,38,38,0.06)', color: '#F87171', fontSize: 12, fontWeight: 600, cursor: 'pointer', transition: 'all 0.15s', whiteSpace: 'nowrap' }}
