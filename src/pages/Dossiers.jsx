    ...
    <div style={{
      ...
      <motion.div
        ...
        style={{
          ...
          borderRadius:20, boxShadow:'0 24px 80px rgba(0,0,0,0.7)', overflow:'hidden',
  ...
  return (
    <Overlay onClose={onClose}>
      ...
      <div style={{ padding:'18px 22px', overflowY:'auto', display:'flex', flexDirection:'column', gap:14 }}>
        ...
        <div>
          ...
          {files.length > 0 && (
            <div style={{ marginTop:8, display:'flex', flexDirection:'column', gap:4 }}>
              {files.map((f, i) => (
                <div key={i} style={{ display:'flex', alignItems:'center', gap:8, padding:'6px 10px', background:'var(--bg-base)', borderRadius:8, fontSize:12 }}>
        ...
        {error &&<p style={{ color:'var(--red,#EF4444)', fontSize:12, padding:'8px 12px', background:'rgba(239,68,68,0.08)', borderRadius:8, margin:0 }}>{error}</p>}
      </div>
...
${filesHtml}${globalHtml}
  ...
  return (
    <Overlay onClose={onClose}>
      ...
      {state === 'done' && (
        <div style={{ padding:'12px 22px', borderTop:'1px solid rgba(255,255,255,0.06)', display:'flex', gap:8, justifyContent:'flex-end', flexShrink:0 }}>
  ...
  return (
    <div style={{ position:'fixed', inset:0, zIndex:1000, background:'rgba(0,0,0,0.5)', backdropFilter:'blur(4px)', display:'flex', alignItems:'center', justifyContent:'center', padding:20 }}
      ...
      <motion.div initial={{ opacity:0, scale:0.96, y:14 }} animate={{ opacity:1, scale:1, y:0 }} transition={{ duration:0.18 }}
        style={{ width:'100%', maxWidth:700, background:'var(--bg-elevated)', border:'1px solid var(--border)', borderRadius:20, boxShadow:'0 24px 80px rgba(0,0,0,0.7)', overflow:'hidden', maxHeight:'90vh', display:'flex', flexDirection:'column' }}>

        {/* Header */}
        <div style={{ padding:'16px 22px', borderBottom:'1px solid rgba(255,255,255,0.06)', display:'flex', alignItems:'flex-start', justifyContent:'space-between', flexShrink:0 }}>
          <div>
        ...
        <div style={{ flex:1, overflowY:'auto', padding:'14px 22px' }}>
          <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:10 }}>
            ...
            <button onClick={() => fileRef.current?.click()} disabled={addingFiles}
              style={{ display:'flex', alignItems:'center', gap:6, padding:'5px 12px', borderRadius:8, border:'1px solid var(--border)', background:'var(--bg-base)', color:'var(--text-secondary)', fontSize:12, cursor:'pointer' }}>
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
            {!initialLoading && dossier.fichiers?.map(f => (
              <div key={f.id} style={{ display:'flex', alignItems:'flex-start', gap:12, padding:'10px 12px', background:'var(--bg-base)', border:'1px solid var(--border)', borderRadius:10 }}>
                ...
                <div style={{ display:'flex', gap:6, flexShrink:0 }}>
                  <button onClick={() => downloadFile(f)} title="Télécharger"
                    style={{ padding:'5px 10px', borderRadius:7, border:'1px solid var(--border)', background:'rgba(255,255,255,0.02)', color:'var(--text-secondary)', fontSize:12, cursor:'pointer' }}>↓</button>
                  <button onClick={() => deleteFile(f.id)} title="Supprimer"
                    style={{ padding:'5px 10px', borderRadius:7, border:'1px solid rgba(239,68,68,0.2)', background:'rgba(239,68,68,0.06)', color:'var(--red,#EF4444)', fontSize:12, cursor:'pointer' }}>×</button>
                </div>
              </div>
            ))}
          </div>
        ...
        {dossier.employes_autorises?.length > 0 && (
          <div style={{ padding:'12px 22px', borderTop:'1px solid rgba(255,255,255,0.06)', flexShrink:0 }}>
            <div style={{ fontSize:12, color:'var(--text-tertiary)', marginBottom:6 }}>Accès accordé à</div>
            <div style={{ display:'flex', gap:6, flexWrap:'wrap' }}>
              {dossier.employes_autorises.map(e => (
                <div key={e.id} style={{ display:'flex', alignItems:'center', gap:6, padding:'4px 10px', background:'var(--bg-base)', border:'1px solid var(--border)', borderRadius:99, fontSize:12, color:'var(--text-secondary)' }}>
  ...
  return (
    <motion.div ref={ref}
      ...
      style={{
        ...
        borderRadius:12, boxShadow:'0 24px 80px rgba(0,0,0,0.7)', padding:'14px 14px 12px',
      }}>
      ...
      {error && <p style={{ color:'var(--red,#EF4444)', fontSize:11, margin:'0 0 8px' }}>{error}</p>}
        ...
        <main style={{ flex:1, padding:'24px 28px', overflowY:'auto' }}>
          ...
          <div style={{ display:'flex', gap:12, marginBottom:22, flexWrap:'wrap' }}>
            {[
              ...
              <div key={s.label} style={{ padding:'10px 16px', background:'var(--bg-elevated)', border:'1px solid var(--border)', borderRadius:10, minWidth:80 }}>
